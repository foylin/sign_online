...
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['str_action']     = ' <a href="' . url("AdminIndex/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="js-ajax-delete" href="' . url("AdminIndex/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
    ...
    public function addCategory($data)
    {
        ...
        if ($result != false) {
            ...
            if (!empty($data['alias']) && !empty($id)) {
                $routeModel->setRoute($data['alias'], 'frame/List/index', ['id' => $id], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'frame/Article/index', ['cid' => $id], 2, 4999);
            }
    ...
    public function editCategory($data)
    {
        ...
        if (empty($oldCategory) || empty($newPath)) {
            ...
            $routeModel = new RouteModel();
            if (!empty($data['alias'])) {
                $routeModel->setRoute($data['alias'], 'frame/List/index', ['id' => $data['id']], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'frame/Article/index', ['cid' => $data['id']], 2, 4999);
            } else {
                $routeModel->deleteRoute('frame/List/index', ['id' => $data['id']]);
                $routeModel->deleteRoute('frame/Article/index', ['cid' => $data['id']]);
            }

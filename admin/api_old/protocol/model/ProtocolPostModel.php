...
class ProtocolPostModel extends CommonModel
{
    ...
    {
        return $this->belongsTo('api\protocol\model\UserModel', 'user_id');
    }
    ...
    {
        return $this->belongsTo('api\protocol\model\UserModel', 'user_id')->field('id,user_nickname');
    }
    ...
    {
        return $this->belongsToMany('api\protocol\model\ProtocolTagModel', 'protocol_tag_post', 'tag_id', 'post_id');
    }
    ...
    {
        return $this->hasOne('api\protocol\model\RecycleBinModel', 'object_id');
    }
        ...
        if (!empty($keywords) || !empty($names)) {
            if (!empty($names)) {
                ...
                if (!empty($shouldDeleteNames)) {
                    ...
                    $tagPosts   = DB::name('protocol_tag_post')->where('tag_id', 'in', $tagIds)
                    ...
                    DB::name('protocolTag')->delete($shouldDeleteTagIds);
                    DB::name('protocolTagPost')->delete($tagPostIds);
                }
            } else {
                $tagIdNames = DB::name('protocol_tag')->where('name', 'in', $keywords)->column('name', 'id');
                if (!empty($tagIdNames)) {
                    $tagIds = array_keys($tagIdNames);
